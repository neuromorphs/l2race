...
def boundaries(im_original, im_copy, b_left, b_right, Sebring=True):
    ...
    # We assume Sebring track which is clockwise. Point with biggest y will hence be on the left hand side
    ...
    # Manual correction for our sand region, just for current implementation of current track
    # left: 0, 1
    # right: 3, 2
    if Sebring & (b_left == 8):
...
boundaries(im, imA, b_left=18, b_right=22)
boundaries(im, imS, b_left=8, b_right=12)
del imA, imS
...
im_start = cv.imread('Start.png', cv.IMREAD_UNCHANGED)
