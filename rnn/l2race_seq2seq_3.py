...
multifeature_csv = pd.read_csv(r'../../data/l2race_4.csv', header=None) 
...
L2Race_Data_car_space = multifeature_csv.iloc[0:nrows, 6:38].values
print("here1", L2Race_Data_car_commands.shape, L2Race_Data_car_space.shape)
before_scaling = np.concatenate((L2Race_Data_car_commands, L2Race_Data_car_space), axis=1)
#print(before_scaling.shape)

# normalize features
scaler = MinMaxScaler(feature_range=(0, 1))
scaled = scaler.fit_transform(before_scaling)
#print(scaled)
print(scaled.shape)
L2Race_Data_car_commands = scaled[:,0:4]   # y 
L2Race_Data_car_space = scaled[:,4:36]     # X

#print("car commands")
#print (L2Race_Data_car_commands)
#print("car space")
#print (L2Race_Data_car_space)
...
#print (L2Race_Data_car_space.shape)     # input

X_train1, X_val1, Y_train1, Y_val1 = train_test_split(L2Race_Data_car_space, L2Race_Data_car_commands, test_size=0.1, shuffle=False)
X_train = X_train1
Y_train = Y_train1
X_val = X_val1
Y_val = Y_val1
...
if (train_flag == 1):
  for epoch in range(N_EPOCHS):
    ...
    if valid_loss < best_valid_loss:
        ...
        torch.save(model.state_dict(), 'l2race-model_test_no_dropout.pt')
...
# Testing 
model.load_state_dict(torch.load('l2race-model_test_no_dropout.pt'))
...
#print(yhat.shape)                     # (10,4)
X = X_val[T1,0:T2,:].numpy()
test_y = Y_val[0,:,:].numpy()
#print(X.shape, test_y.shape)          # (10,32)  (10,4)
# invert scaling for prediction
y_hat_inv = scaler.inverse_transform(np.concatenate((yhat,X),axis=1))
# invert scaling for actual
y_inv = scaler.inverse_transform(np.concatenate((test_y,X),axis=1))
#print(y_hat_inv.shape, y_inv.shape)   # (10,36) (10,36)
#print(y_hat_inv)
#print(y_inv)
